...
global nbtn, xpos, ypos
nbtn = 0; xpos = 0.1; ypos = 0.85
global abtn; abtn = []
global afnc; afnc = []
import time
...
class IDLE(BoxLayout):
        ...
        def dbv2(self, instance):
                ...
                self.dbtn.bind(on_release=self.DynamicClear)
        ...
        def repaintClear(self):
                programarea = self.ids.areamodulesprogram
                for l in abtn:
                        print(l.id, "   ", abtn[int(l.id)], "   ", l)
                        programarea.remove_widget(l)
